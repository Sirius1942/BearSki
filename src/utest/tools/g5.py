...
import time
import json
from hartool.core import HarParser
...
class MY_GUI():
    ...
    def getOneMessage(self,req):
        # req,res="",""
        result,res=self.getMessage(req)
    ...
    def createAllCase(self):
        for onereq in self.harfile:
            result,res=self.getMessage(self.harfile[onereq])
    ...
    def getMessage(self,req):
        ...
        if req:
            try:
                hp=HarParser("./")
                result=hp.getOneRequest(req)
                # print(result)
                # request_str=req
                # respone_str=result
                respone_str=req['response']
                return result,respone_str
